...
public sealed class WindowsAutostartProvider : IAutostartProvider
{
    ...
    public bool IsAutostartEnabled
    {
        ...
        {
            if (IsReady)
                return _autostartRegistryKey!.GetValue("BambiHeavy") != null;
            return false;
        }
    }
    ...
    {
        if (IsReady) _autostartRegistryKey!.SetValue("BambiHeavy", _binaryLocation!);
    }

    public void DisableAutostart()
    {
        if (IsReady) _autostartRegistryKey!.DeleteValue("BambiHeavy", false);
    }
}